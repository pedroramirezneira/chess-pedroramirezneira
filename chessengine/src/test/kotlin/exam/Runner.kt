...
class Runner(
    private val game: IGame = createChess(),
) : TestGameRunner {
    override fun executeMove(
        ...
        return when {
            ...
            state is GameEnded && state.currentPlayer -> WhiteCheckMate(Runner(state).getBoard())
            state is GameEnded && !state.currentPlayer -> BlackCheckMate(Runner(state).getBoard())
    ...
    companion object {
        infix fun toGame(board: TestBoard): Game {
