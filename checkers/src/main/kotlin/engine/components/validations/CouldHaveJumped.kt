...
val couldHaveJumped = Validation { game ->
    when {
        game.states.isEmpty() -> true
        else -> {
            ...
            val previousPosition = previousState.board.getPieces().find { tile -> tile.piece === movedPosition.piece }
                        ...
                        .any { tile ->
                            (0 until previousState.board.size.height).any { y ->
                                (0 until previousState.board.size.width).any { x ->
                                    ...
                                    val couldHaveJumped =
                                        ManJump().verify(coordinates, previousState) || KingJump().verify(
                                            coordinates,
                                            game
                                        )
